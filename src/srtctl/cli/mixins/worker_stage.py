...
def _build_nsys_output_path(mode: str, node: str, index: int, *, rank_qualified: bool = False) -> str:
    ...
    Nsight Systems expands %q{VAR} from the target process environment. MPI-style
    launches run one profiler instance per Slurm rank, so rank-qualified names
    prevent multiple ranks from writing the same report file.
    """
    rank_suffix = ""
    if rank_qualified:
